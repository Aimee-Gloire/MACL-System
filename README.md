...
MACL shows that distributed, rule-based verification can replace single-custodian trust in NGO programme accountability: the contracts make agreements immutable, evaluate compliance identically for every party, require multi-party agreement to finalise and to approve spend, and let each organisation independently check that every node holds the same record — all driven from a dashboard usable by non-technical staff. The RQ3 comparison against a centralised PostgreSQL baseline has been carried out (see `evaluation/`): MACL detects a tampered figure across nodes in milliseconds where the centralised store cannot detect it at all, keeps a complete audit trail where the baseline overwrites in place, and recovers a diverged node to the majority chain automatically. The full results table is in `evaluation/results/metrics.md`.
